...
export default function ReservationList() {
  const [reservations, isLoading, getReservations] = useReservation();
  console.log(reservations, isLoading, getReservations);
  ...
  const onDeleteHandler = async (id: string) => {
    try {
      ...
      if (success) {
        await getReservations(); // Fetch updated reservations only on success
      }
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        ...
        <TableContainer>
          <Table key={reservations ? reservations.length : 0}>
            <TableHead>
              <TableRow>
